...
Position traziprev(char[], Position);
...
int main() {
	...
	while (1) {
		...
		switch (x) {
		case 0:
			printf("gotovo je unosenje brojeva\n");
			break;
		case 1:
			...
			if (y == 1) {
				...
				scanf("%s", namedir);
				scanf("%s", namepoddir);
				unospoddirektorija(namepoddir, namedir, Head,start);
			}
			break;
		case 2:
			printf("unesi naziv direktorija/poddirektorija koji zelis naci\n");
			scanf("%s", namedir);
			p=trazi(namedir, Head,start);
			printf("nadoh,%s\n", p->name);
			break;
		case 3:
			prev = traziprev(p->name, Head);
...
Position traziprev(char x[DIR_NAME_LENGHT], Position p) {
	Position temp, prev;
	if (p->child == NULL) {
		printf("Greska, direktorij je prazan\n");
		return NULL;
	...
	printf("Greska, nema prethodnog direktorija imenom\n");
	return NULL;
}
